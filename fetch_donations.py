...
# =====================================================================
# DONOR VIEW CREATION (per-page filtered table)
# ---------------------------------------------------------------------
# Every new donation page gets its own linked-database view, filtered to
# the donor's email, so the "Donations Follow-Up" table on that page shows
# only that donor's history (Notion templates cannot substitute "This page"
# into a filter after the fact, so each page needs its own view created
# via the API with the filter baked in at creation time).
#
# NOTION_DATA_SOURCE_ID: the data source backing the Donations database —
# this is what the new view's rows are pulled from.
# SOURCE_VIEW_ID: an existing, manually-configured view whose column
# layout (visibility, order, width, sort) we copy onto every new view, so
# they all look consistent with each other.
# =====================================================================
NOTION_DATA_SOURCE_ID = "b9069e0c-af07-47c6-ac1f-ba84341f8b75"
SOURCE_VIEW_ID = "aa56d27fdd65437fada60d872b494223"
...
# =====================================================================
# PER-DONOR VIEW (filtered "Donations Follow-Up" table on each page)
# =====================================================================

def get_reference_view_config():
    """Read the column layout (visibility, order, width) and sort order off
    an existing, manually-configured view, so every new per-donor view we
    create looks the same as it."""
    response = requests.get(
        f"https://api.notion.com/v1/views/{SOURCE_VIEW_ID}",
        headers=NOTION_HEADERS
    )
    if response.status_code != 200:
        print(f"WARNING: could not read reference view config "
              f"({response.status_code}); new views will use default layout")
        return None, None
    data = response.json()
    return data.get("configuration"), data.get("sorts")


def create_donor_view(page_id, donor_name, donor_email, reference_config, reference_sorts):
    """Create a new linked-database view on this donation page, filtered to
    this donor's email, with the same look as the reference view.

    Each page gets its OWN view (rather than editing one shared view),
    because Notion cannot substitute "This page" into a filter after the
    template has already been applied — creating a fresh view per page with
    the filter baked in sidesteps that limitation entirely.
    """
    config = None
    if reference_config:
        config = dict(reference_config)
        # The API rejects frozen_column_index == -1 on creation (the value a
        # "nothing frozen" view reports back) — it wants >= 0 or absent.
        if config.get("frozen_column_index") == -1:
            config.pop("frozen_column_index")
        # Drop columns that reference properties no longer in the schema —
        # the reference view silently ignores these, but view creation
        # validates them strictly and rejects the whole request otherwise.
        if config.get("properties"):
            config["properties"] = [
                p for p in config["properties"] if p.get("property_name")
            ]

    payload = {
        "create_database": {"parent": {"type": "page_id", "page_id": page_id}},
        "data_source_id": NOTION_DATA_SOURCE_ID,
        "name": "Donations Follow-Up",
        "type": "table",
    ...
    if reference_sorts:
        payload["sorts"] = reference_sorts

    if DRY_RUN:
        print(f"DRY RUN — would create donor view for {donor_name} ({donor_email})")
        return

    response = requests.post("https://api.notion.com/v1/views", headers=NOTION_HEADERS, json=payload)
    if response.status_code not in (200, 201):
        print(f"WARNING: failed to create donor view for {donor_name} "
              f"({donor_email}): {response.status_code} {response.text}")
        return

    print(f"Created donor view for {donor_name} ({donor_email})")


def write_to_notion(zoho_id, donor_name, donor_email, amount, date,
                    donor_status, designation, utm,
                    reference_config, reference_sorts):
    properties = {
        "Donor Name": {
    ...
    # Give this new page its own filtered donor-history table.
    if donor_email:
        create_donor_view(page_id, donor_name, donor_email, reference_config, reference_sorts)
...
def main():
    ...
    print("Reading reference view configuration...")
    reference_config, reference_sorts = get_reference_view_config()
