...
# =====================================================================
# DONOR HISTORY (relation + template)
# ---------------------------------------------------------------------
# Self-relation on the Donations database (two-way), linking every donation
# to all other donations with the same Donor Email. The name must match the
# Notion property exactly. The "Donation card template" (default template)
# contains a linked view filtered by this relation, so each donation page
# shows the donor's past donations under the comments.
# =====================================================================
...
def find_pages_by_email(email):
    """IDs of all existing Notion pages with this donor email.

    Used to fill the two-way self-relation, so every donation links to the
    donor's other donations.
    """
    if not email:
        return []
    ids, cursor = [], None
    while True:
        payload = {
            "filter": {"property": "Donor Email", "email": {"equals": email}},
            "page_size": 100
        }
        if cursor:
    ...
    return ids[:100]  # Notion accepts at most 100 relations per write


def apply_template(page_id):
    """Apply the database's default template (linked view of past donations).

    Pages created through the API do not get a template automatically, so it
    is applied right after creation. Uses a newer Notion-Version for this one
    call only; everything else stays on the old version.
    """
    response = requests.patch(
        f"https://api.notion.com/v1/pages/{page_id}",
        headers={**NOTION_HEADERS, "Notion-Version": "2026-03-11"},
        json={"template": {"type": "default", "timezone": "Europe/Kyiv"}}
    )
    if response.status_code != 200:
        print("Template not applied:", response.status_code, response.text)
...
def write_to_notion(zoho_id, donor_name, donor_email, amount, date,
    ...
    # Link this donation to the donor's other donations (two-way relation,
    # so the older pages point back to this one automatically).
    if donor_email and not DRY_RUN:
        related = find_pages_by_email(donor_email)
        if related:
            properties[RELATED_PROPERTY] = {"relation": [{"id": i} for i in related]}
    ...
    # Same for the relation property: if it was renamed or removed, write the
    # donation without it instead of failing the whole run.
    if response.status_code == 400 and RELATED_PROPERTY in properties \
            and RELATED_PROPERTY in response.text:
        print(f"WARNING: Notion has no '{RELATED_PROPERTY}' property — "
              f"writing without it.")
        properties.pop(RELATED_PROPERTY)
        payload["properties"] = properties
        response = requests.post(
            "https://api.notion.com/v1/pages",
            headers=NOTION_HEADERS,
    ...
    # API-created pages don't get the template on their own; apply it now.
    apply_template(response.json()["id"])
